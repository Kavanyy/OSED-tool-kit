...
def main():
    ...
    ap.add_argument("-t", "--tag", required=True, help="TAG de 4 caracteres (ej: w00t)")
    ...
    ap.add_argument("-n", "--negate", action="store_true", help="Usar NEG para evitar null bytes")
    ap.add_argument("-b", "--badchars", default="", help=r'Badchars: "\x00\x0a"')
    ap.add_argument("-f", "--format", choices=["python", "c", "hex"], default="python")
    ap.add_argument("-o", "--outfile", help="Guardar en fichero")
    args = ap.parse_args()

    if len(args.tag) != 4 or not args.tag.isascii():
        sys.exit("ERROR: TAG debe tener exactamente 4 caracteres ASCII.")
